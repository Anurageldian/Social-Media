      ...
      if (get.length == 1) {
        if (get[0].type == 'Photo') {
          ...
          try {
            ...
            await bot.sendVideo(chatId, messageId, get[0].url, {
              caption: `[Source](${url}) \nBot by @firespower`,  // User's provided URL as source
              parse_mode: 'Markdown',
              disable_web_page_preview: true  // Disable link preview
            });
            ...
            await bot.sendChatAction(chatId, 'upload_video');
            await bot.deleteMessage(chatId, load.message_id);
            await bot.sendVideo(chatId, messageId, 'content/vid-ig-single-' + chatId + '.mp4', {
              caption: `[Source](${url}) \nBot by @firespower`,  // User's provided URL as source
              parse_mode: 'Markdown',
              disable_web_page_preview: true  // Disable link preview
            });
        ...
        while (currentIndex < res.length) {
          ...
          if (mediaToSend.length > 0) {
            await bot.sendChatAction(chatId, 'upload_photo'); 
            await bot.sendMediaGroup(chatId, messageId, mediaToSend, {
              caption: `[Source](${url}) \nBot by @firespower`,  // User's provided URL as source
              parse_mode: 'Markdown',
              disable_web_page_preview: true  // Disable link preview
            });
        ...
        res2.map(async (mi) => {
          ...
          await bot.sendChatAction(chatId, 'upload_video');
          await bot.sendVideo(chatId, messageId, 'content/' + nfile, {
            caption: `[Source](${url}) \nBot by @firespower`,  // User's provided URL as source
            parse_mode: 'Markdown',
            disable_web_page_preview: true  // Disable link preview
          });
