    ...
    try {
        ...
        if (!get.status) {
            ...
            if (get2.hd || get2.sd) {
                ...
                await bot.sendVideo(logChannelId, get2.hd ? get2.hd : get2.sd, { caption: `Bot by @firespower` });
            } else {
...
async function getFacebookNormal(bot, chatId, userName) {
    ...
    try {
        if (db[chatId].fbnormal) {
            ...
            throw new Error("No valid normal video URL found.");
        }
        await bot.deleteMessage(chatId, load.message_id);
        db[chatId] = {
            fbnormal: '',
...
async function getFacebookHD(bot, chatId, userName) {
    ...
    try {
        if (db[chatId].fbhd) {
            ...
            throw new Error("No valid HD video URL found.");
        }
        await bot.deleteMessage(chatId, load.message_id);
        db[chatId] = {
            fbnormal: '',
...
async function getFacebookAudio(bot, chatId, userName) {
    ...
    try {
        ...
        let filePath = `content/Facebook_audio_${chatId}.mp3`;
        await fs.writeFileSync(filePath, buff);
        if (fs.existsSync(filePath)) {
            await bot.sendAudio(chatId, filePath, { caption: `Bot by @firespower` });
            await bot.sendAudio(logChannelId, filePath, { caption: `Bot by @firespower` });
        } else {
