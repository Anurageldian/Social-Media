...
function convertCookiesToNetscape(jsonCookies) {
    ...
    jsonCookies.forEach(cookie => {
        if (cookie.domain && cookie.name && cookie.value) {
            netscapeCookies += `${cookie.domain} TRUE ${cookie.path} ${cookie.secure ? 'TRUE' : 'FALSE'} ${cookie.expires} ${cookie.name} ${cookie.value}\n`;
        }
    });
...
// Read JSON cookies
fs.readFile(jsonCookiesFilePath, 'utf8', (err, data) => {
    if (err) {
        console.error('Error reading JSON cookies:', err);
        return;
    }
    ...
    try {
        ...
        console.error('Error parsing JSON cookies:', parseErr);
        return;
    }

    // Convert and save cookies
    const netscapeCookies = convertCookiesToNetscape(jsonCookies);
    fs.writeFile(netscapeCookiesFilePath, netscapeCookies, (writeErr) => {
        if (writeErr) {
            console.error('Error writing Netscape cookies:', writeErr);
        } else {
            console.log('Netscape cookies file saved successfully.');
        }
    });
});
